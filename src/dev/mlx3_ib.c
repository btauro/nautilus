...
mlx3_query_dev_cap (struct mlx3_ib * mlx)
{
    ...
    MLX3_GET(field, outbox, QUERY_DEV_CAP_MAX_SG_RQ_OFFSET);
    cap->max_sg_rq = field;
    MLX3_GET(size, outbox, QUERY_DEV_CAP_MAX_DESC_SZ_RQ_OFFSET);
    cap->max_desc_sz_rq = size;
    ...
    MLX3_GET(field, outbox, QUERY_DEV_CAP_BF_OFFSET);
    if (field & 0x80) {
        MLX3_GET(field, outbox, QUERY_DEV_CAP_LOG_BF_REG_SZ_OFFSET);
        cap->bf_reg_size = 1 << (field & 0x1f);
        MLX3_GET(field, outbox, QUERY_DEV_CAP_LOG_MAX_BF_REGS_PER_PAGE_OFFSET);
        if ((1 << (field & 0x3f)) > (PAGE_SIZE_4KB / cap->bf_reg_size))
            field = 3;
        cap->bf_regs_per_page = 1 << (field & 0x3f);
    } else {
        cap->bf_reg_size = 0;
    }

    MLX3_GET(field, outbox, QUERY_DEV_CAP_LOG_MAX_BF_PAGES_OFFSET);
    DEBUG("Max BF Pages %d \n", ( 1 << (field & 0x3f)));
...
static inline void
writeq (uint32_t b[2], uint64_t *addr)
{
    *(volatile uint64_t*)addr = *(volatile uint64_t*)b;
}
                                ...
                                uint8_t eqe_size)
{
    ...
    return eq->buffer + (offset + (eqe_factor ? MLX3_EQ_ENTRY_SIZE : 0)) % eq->size;
}
...
{
    return XNOR((cqe->owner_sr_opcode & 0X80), (cq->cons_index & cq->nentry));
}
...
mlx3_cq_arm (struct mlx3_cq * cq, uint32_t cmd, void * uar_page, int cons_index)
{
    ...
    *cq->arm_db = bswap32(sn << 28 | cmd | ci);
    ...
    writeq(doorbell, uar_page + MLX3_CQ_DOORBELL);
...
get_qp_counters (struct mlx3_ib * mlx, struct mlx3_qp * qp, struct mlx3_qp_context * context, mlx3_cmd_box_t * mailbox)
{
  //  uint64_t start = rdtsc();
    ...
    context->sq_wqe_counter = -1;
    context->rq_wqe_counter = -1;
    ...
    context->sq_wqe_counter = bswap16(context->sq_wqe_counter);
    context->rq_wqe_counter = bswap16(context->rq_wqe_counter);
//    uint64_t end = rdtsc();
}
...
mlx3_attach_sq (struct mlx3_ib * mlx, struct mlx3_qp * qp, int size, int stride, void * buf_start) 
{
    ...
    swq->buf_size   = size; //asuming one basic block to be a single wqe
    swq->buf        = buf_start;
                ...
                int nent)
{
    ...
    npages = PAGE_ALIGN((CQE_SIZE * nent)) / PAGE_SIZE_4KB;    
    printk("napges %d\n", npages);
    cq->size = npages * PAGE_SIZE_4KB;

    mtt_addr = mlx3_alloc_mtt(mlx, npages);

    __mzero_checked(buf, ((CQE_SIZE * nent) + (CQE_SIZE - 1)), "Could not allocate CQ buffer\n", goto out_err1);

    cq->buffer = buf;
    byte_align(cq->buffer, CQE_SIZE);
...
create_qp (struct mlx3_ib * mlx, struct mlx3_cq * cq, int size, int is_sqp, int transport_ctx)
{
    ...
    byte_align(qp->db_rec , QP_DB_SIZE);
    DEBUG("DB REC %p\n", db_rec);
...
static inline void 
bf_copy(uint64_t *dst, uint64_t *src, unsigned int bytecnt)
{
